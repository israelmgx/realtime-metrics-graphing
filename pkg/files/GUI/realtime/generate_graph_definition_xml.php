...
// check if seriesname includes more than one
$seriesnames = array($seriesname);
if ( contains(",", $seriesname) ) {
	$seriesnames = preg_split('/,/', $seriesname);
}
...
foreach ($seriesnames as $name) {
	$name = trim($name);
	echo "	<dataset seriesName='{$name}' showValues='0'></dataset>\n";
}
?>
	<styles>
		<definition>
